...
use crate::{
    LIGHTER_DEPOSIT_CONTRACT_ADDRESS, LIGHTER_ROUTE_TYPE_PERP, LIGHTER_USDC_ASSET_INDEX,
};
...
fn encode_deposit_call(
    ...
    let call = ILighterDeposit::depositCall {
        deposit: U256::ZERO,
        _to: user,
        _assetIndex: U256::from(asset_index),
        _routeType: U256::from(route_type),
...
async fn ensure_permit(
    ...
    if has_sufficient_balance(&allowance, &U256::from(amount)) {
        log::info!("Existing USDC allowance sufficient for Lighter deposit, skipping permit");
        return Ok(());
    }

    let call_data = encode_permit_call(user, spender, amount, permit)?;
    ...
    log::info!(
        "USDC permit executed for user {user} spender {spender} amount {amount}, tx: {:?}",
